...
module.controller('KbnTimeVisController', function (config, $scope, $rootScope, Private, $filter, $timeout, timefilter) {
  ...
  $scope.format = 'MMMM Do YYYY, HH:mm:ss';
  ...
  $scope.day_in_million_seconds = 86400000;
  ...
  $scope.calendarClick = function (event) {
    console.log("Calendar Submit");
    $scope.time.absolute_from = moment($scope.time.calendar_from);  //moment($scope.time.from)
    console.log($scope.time);
    $scope.time.absolute_to = moment($scope.time.absolute_from).endOf('day');
    console.log(event);
  }
  ...
  $scope.applyAbsolute = function () {
    ...
    $scope.time.from = moment($scope.time.to).subtract(1, 'day');
    updateKbnTime();
  }
  ...
  $scope.setAbsolute = function () {
    ...
    expectedFrom = $scope.time.from;
    expectedTo = $scope.time.to;
    console.log($scope.time.absolute_from);
    console.log($scope.time.absolute_from - $scope.time.absolute_to);
    updateKbnTime();
  };
